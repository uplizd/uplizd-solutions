...
# Team Productivity Monitor (Clockify) - Analyze Performance & Workload Balance
...
- **Resource Utilization Analysis**  
  Highlights team members with low project involvement (less than 30 hours/week) to reveal hidden capacity.

- **Bottleneck Detection**  
  Flags individuals juggling more than 5 concurrent projects, helping to prevent task-switching overhead and delivery delays.

- **Organizational Hierarchy Integration**  
  Maps reporting structures to ensure productivity insights are delivered within the correct organizational context.
...
4. Ensure all nodes are connected correctly:
   - **Chat Input**
   - **Clockify - CLOCKIFY_FIND_ALL_USERS_ON_WORKSPACE**
   - **Clockify - CLOCKIFY_FILTER_WORKSPACE_USERS**
   - **Clockify - CLOCKIFY_FIND_USER_TEAM_MANAGER**
   - **Agent**
   - **Chat Output**
...
- **Chat Input** → receives commands for workspace analysis or specific team reports.
- **Agent** → coordinates the 5-step analysis workflow (Data Retrieval -> Segmentation -> Hierarchy Mapping -> Pattern Analysis -> Reporting).
- **Clockify Toolset** → provides the primary data source for time and project metrics.
- **Chat Output** → displays structured productivity insights and executive recommendations.
...
2. Enter a request such as:
   ...
   - `"Who is working on more than 5 projects at once?"`
...
### 2) Clockify Toolset Nodes
Requires your **Composio API Key** and a synchronized connection to your **Clockify** workspace.

### 3) Tool Availability
The agent can call tools for:
- Workspace user discovery
- Metric-based user filtering
- Management structure verification
